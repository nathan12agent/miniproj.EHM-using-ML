...
function DoctorAppointments() {
  ...
  const [billingDialog, setBillingDialog] = useState({ open: false, apt: null });
  ...
  const handleCompleteAndBill = async () => {
    if (!billingDialog.apt) return;
    setBillingLoading(true);
    try {
      const apt = billingDialog.apt;
      const fee = apt.consultationFee ?? apt.doctor?.consultationFee ?? 0;

      await schedulingAPI.updateAppointmentStatus(apt._id, 'Completed');

      await billingAPI.create({
        patient: apt.patient._id,
        appointment: apt._id,
        items: [{
          description: `${apt.type || 'Consultation'} — Dr. ${apt.doctor?.firstName} ${apt.doctor?.lastName}`,
          quantity: 1,
          unitPrice: fee,
          total: fee,
        }],
      ...
      setBillingDialog({ open: false, apt: null });
      setBillingNote('');
      loadAppointments();
    } catch (err) {
  ...
  return (
    <Box>
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            ...
                            {['Scheduled', 'Confirmed', 'In Progress'].includes(apt.status) && (
                              <Button
                                ...
                                onClick={() => { setBillingDialog({ open: true, apt }); setBillingNote(''); }}
                                sx={{ backgroundColor: '#10b981', '&:hover': { backgroundColor: '#059669' }, fontSize: 11, py: 0.5, px: 1 }}
                              >
                                Complete & Bill
                              </Button>
                            )}
      ...
      <Dialog open={billingDialog.open} onClose={() => setBillingDialog({ open: false, apt: null })} maxWidth="xs" fullWidth>
        <DialogTitle sx={{ fontWeight: 700 }}>Complete Appointment & Forward to Billing</DialogTitle>
        <DialogContent>
          <Typography variant="body2" sx={{ mb: 1 }}>
            Patient: <strong>{billingDialog.apt?.patient?.firstName} {billingDialog.apt?.patient?.lastName}</strong>
          </Typography>
          <Typography variant="body2" sx={{ mb: 2 }}>
            Fee: <strong>₹{billingDialog.apt?.consultationFee || billingDialog.apt?.doctor?.consultationFee || 0}</strong>
          </Typography>
          <TextField
            fullWidth
            multiline
        ...
        <DialogActions>
          <Button onClick={() => setBillingDialog({ open: false, apt: null })} sx={{ color: '#6b7280' }}>Cancel</Button>
          <Button
            variant="contained"
          ...
          >
            {billingLoading ? 'Processing...' : 'Confirm & Bill'}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
