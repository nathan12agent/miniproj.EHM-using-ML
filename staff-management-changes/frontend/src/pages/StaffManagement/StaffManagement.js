...
import { doctorsAPI, nursesAPI, attendanceAPI, patientsAPI } from '../../services/api';
...
const bedBadge = (required) =>
  required
    ? { color: 'error', label: 'Bed Required', icon: <BedIcon fontSize="small" /> }
    : { color: 'success', label: 'Outpatient Only', icon: <OkIcon fontSize="small" /> };
...
function SmartAssignPanel() {
  ...
  const badge = bedBadge(result?.bedRequired ?? SEVERITIES.indexOf(severity) > 0);
        ...
        {result && (
          ...
          >
            ...
            <Box sx={{ mt: 1, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
              <Chip
                size="small"
                label={result.wardInfo?.careType}
                color={result.bedRequired ? 'warning' : 'success'}
              />
              <Chip
                size="small"
                label={result.bedRequired ? '🛏 Bed Required' : '✓ No Bed Needed'}
                variant="outlined"
                color={result.bedRequired ? 'error' : 'success'}
              />
              {result.assignedNurse && (
                <Chip
                  size="small"
                  label={`Nurse: ${result.assignedNurse.firstName} ${result.assignedNurse.lastName}`}
                  color="info"
                />
              )}
              {!result.assignedNurse && (
