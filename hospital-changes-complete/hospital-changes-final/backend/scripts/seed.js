...
const seedDatabase = async () => {
  try {
    ...
    const existingPatients = await Patient.find().limit(9);
    if (existingPatients.length > 0) {
      // Assign first 3 patients to ICU beds
      for (let i = 0; i < Math.min(3, existingPatients.length); i++) {
        const bed = createdBeds.find(b => b.bedNumber === `ICU-10${i + 1}`);
        if (bed) {
          bed.patient = existingPatients[i]._id;
          bed.status = 'Occupied';
      ...
      for (let i = 3; i < Math.min(7, existingPatients.length); i++) {
      ...
      for (let i = 7; i < Math.min(9, existingPatients.length); i++) {
